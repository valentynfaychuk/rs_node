...
// Trait for socket-like objects that can receive packets
...
// Implementation for UdpSocket
impl PacketReceiver for UdpSocket {
    fn recv_from<'a>(
        &'a self,
        buf: &'a mut [u8],
    ) -> Pin<Box<dyn Future<Output = io::Result<(usize, SocketAddr)>> + Send + 'a>> {
        Box::pin(self.recv_from(buf))
    }
}

// LogSocket for reading from capture files
use std::fs::File;
...
impl LogSocket {
    ...
    fn read_next_datagram(&self, buf: &mut [u8]) -> io::Result<(usize, SocketAddr)> {
        let mut reader = self.reader.lock().unwrap();

        // Read all remaining bytes into a buffer
        let mut data = Vec::new();
        let bytes_read = reader.read_to_end(&mut data)?;

        if bytes_read == 0 {
            // End of file reached, return WouldBlock to allow timeout behavior
            return Err(io::Error::new(io::ErrorKind::WouldBlock, "End of file reached"));
        }

        // Try to decode one UdpLoggedDatagram from the beginning
        let (datagram, consumed) =
            bincode::decode_from_slice::<UdpLoggedDatagram, _>(&data, bincode::config::standard())
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;

        // Seek to after the consumed bytes for next read
        ...
        Ok((copy_len, datagram.src()))
    }
}

impl PacketReceiver for LogSocket {
    fn recv_from<'a>(
        &'a self,
        buf: &'a mut [u8],
    ) -> Pin<Box<dyn Future<Output = io::Result<(usize, SocketAddr)>> + Send + 'a>> {
        Box::pin(async move {
            loop {
                match self.read_next_datagram(buf) {
                    Ok(result) => return Ok(result),
                    Err(e) if e.kind() == io::ErrorKind::WouldBlock => {
                        // EOF reached, block indefinitely like UdpSocket would when no packets available
