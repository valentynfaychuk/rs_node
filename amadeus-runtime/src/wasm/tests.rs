...
fn setup_test_db(name: &str) -> rocksdb::RocksDb {
    let base = format!("{}/{}_{}", TEST_DB, name, std::process::id());
    let rt = tokio::runtime::Runtime::new().expect("create runtime");
    rt.block_on(rocksdb::RocksDb::open(base)).expect("open test db")
}
